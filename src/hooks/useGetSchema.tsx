...
function ReplaceEnumRefs (schemas: Record<string, Schema>, schemaName: string): Schema {
    ...
    if (foundSchema && 'properties' in foundSchema) {
        ...
        Object.entries(foundSchema.properties).forEach(([key, value]) => {
            if ('schema' in value && value.schema) {
                ...
                if (schemas[refName] && 'enum' in schemas[refName] && 'type' in schemas[refName]) {
...
function InheritEntities (foundSchema: Schema, schemas: Record<string, Schema>): EntitySchema {
