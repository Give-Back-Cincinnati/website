...
const EventsAdmin: NextPageWithLayout = () => {
    ...
    const { items, actions, filteredItemsCount, collectionProps, filterProps, paginationProps } = useCollection(events || [], {
        filtering: {
            empty: '',
    ...
    return <div>
        {
            events && canSeeEvents
                ? <Table
                    {...collectionProps}
                    header={<Header
                            counter={filteredItemsCount ? `(${filteredItemsCount?.toString()})` : ''}
                            actions={canCreateEvent && schema && <>
                    ...
                    columnDefinitions={[
                        ...
                        { id: 'dateTime', header: 'Date & Time', cell: e => <>
                            <Box>
                                Starts: { DateTime.fromISO(e.startTime).toLocaleString(DateTime.DATETIME_MED_WITH_WEEKDAY) }
                            </Box>
                            <Box>
                                Ends: { DateTime.fromISO(e.endTime).toLocaleString(DateTime.DATETIME_MED_WITH_WEEKDAY) }
                            </Box>
                        </> },
                        { id: 'registrations', header: 'Num Registrants', cell: e => e._id && formattedEventRegistrations[e._id] },
                        { id: 'maxRegistrations', header: 'Max Registrants', cell: e => e.maxRegistrations },
                    ]}
